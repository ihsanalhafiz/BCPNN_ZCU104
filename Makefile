...
VPP := v++
CMD_ARGS = $(BUILD_DIR)/BCPNN_Kernel.xclbin
SDCARD := sd_card

include $(XF_PROJ_ROOT)/common/includes/opencl/opencl.mk
CXXFLAGS += $(opencl_CXXFLAGS) -Wall -O3 -g -std=c++17 `pkg-config --cflags opencv4` -pthread
LDFLAGS += $(opencl_LDFLAGS) `pkg-config --libs opencv4` -pthread
...
HOST_OBJS := ./bin/Parseparam.o \
             ...
             ./bin/Analys.o \
             ./bin/Probe.o \
			 ./bin/BCPNN_Kernel_HLS.o

CCFLAGS := -std=c++17 -O3 -g -I./libsrc/include/
...
VPP_FLAGS_HLS := --kernel_frequency 150
...
BINARY_CONTAINERS += $(BUILD_DIR)/BCPNN_Kernel.xclbin
BINARY_CONTAINER_adder_OBJS += $(TEMP_DIR)/BCPNN_Kernel.xo
...
$(TEMP_DIR)/BCPNN_Kernel.xo: libsrc/src/BCPNN_Kernel.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(VPP_FLAGS) $(VPP_FLAGS_CONFIG) $(VPP_FLAGS_HLS) -c -k BCPNN_Kernel --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<' --hls.pre_tcl ./compile_hls.tcl -I ./libsrc/include/
$(BUILD_DIR)/BCPNN_Kernel.xclbin: $(BINARY_CONTAINER_adder_OBJS)
	mkdir -p $(BUILD_DIR)
ifeq ($(HOST_ARCH), x86)
	$(VPP) $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(BUILD_DIR)  -o'$(BUILD_DIR)/BCPNN_Kernel.link.xclbin' $(+) -I ./libsrc/include/
	$(VPP) -p $(BUILD_DIR)/BCPNN_Kernel.link.xclbin -t $(TARGET) --platform $(DEVICE) --package.out_dir $(PACKAGE_OUT) -o $(BUILD_DIR)/BCPNN_Kernel.xclbin -I ./libsrc/include/
else
	$(VPP) $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(BUILD_DIR) -o'$(BUILD_DIR)/BCPNN_Kernel.xclbin' $(+) -I ./libsrc/include/
endif
...
ifneq ($(HOST_ARCH), x86)
	$(VPP) -p $(BUILD_DIR)/BCPNN_Kernel.xclbin -t $(TARGET) --platform $(DEVICE) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) -o BCPNN_Kernel.xclbin
endif
