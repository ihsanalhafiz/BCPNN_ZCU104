    ...
    for (int rep = 0, ncorr; rep < nrep; rep++) {
        ...
        while (true) {
            ...
            if (!square.empty()) {
                ...
                // Save warp into a std::vector<float> of size 28x28 (flattened)
                
                // Assume warp is a CV_8UC1 matrix; convert each pixel to float normalized between 0 and 1.
                ...
                // Create wrap_binarize with size 2x28x28 following the provided binarization process
            ...
            std::string text = "Class: " + std::to_string(counter);
            counter++;
