...
struct timeval train_time;
struct timeval inference_time;
float train_time_single = 0.0;
...
int main(int argc, char **args) {
    ...
    for (int rep = 0, ncorr; rep < nrep; rep++) {
        ...
        for (int epoch = 0; epoch < nepoch; epoch++) {
            ...
            for (int p = 0, q; p < trnpat; p++) {
                ...
                modeOps = UNSUPERVISED;
                gettimeofday(&train_time, 0);
                ...
                for(int i = 0; i < N_in; i++) {
                    inputdata[i] = trainpatfac->getfpat(q)[i];
                }
                for(int i = 0; i < N_ut; i++) {
                    labeldata[i] = trainlblfac->getfpat(q)[i];
                }
                setKernelArguments(
                ...
                copyVectorToData(ihprj, huprj, Pj_ih_cl, Bj_ih_cl, Pi_ih_cl, Pji_ih_cl, Wji_ih_cl,
                                 Pj_hu_cl, Bj_hu_cl, Pi_hu_cl, Pji_hu_cl, Wji_hu_cl, needsupdbw_cl);
                train_time_single += getDiffTime(train_time);
        ...
        for (int p = 0; p < trnpat; p++) {
            ...
            for(int i = 0; i < N_ut; i++) {
                labeldata[i] = trainlblfac->getfpat(p)[i];
            }
            setKernelArguments(
                krnl_bcpnn, qq, modeOps, buf_inputdata, buf_labeldata, buf_outputdata, buf_rndPoisson_hid,
                buf_Hihjhi_ih, buf_Chjhi_ih, buf_needsupdbw, buf_Pj_ih, buf_Pi_ih, buf_Pji_ih, buf_Bj_ih,
        ...
        copyDataToBuffer(inpop, hidpop, utpop, ihprj, huprj, SUPERVISED);
        saveVectorsToFile(trained_data_file);
        ...
        for (int p = 0; p < tenpat; p++) {
            ...
            gettimeofday(&inference_time, 0);
             ...
             infer_time_single += getDiffTime(inference_time);
    ...
    if (myverbosity > 1) {
        ...
        printf("Train time = %.3f ms, Infer time = %.3f ms\n", (train_time_single/(trnpat*nepoch)), infer_time_single/tenpat);
